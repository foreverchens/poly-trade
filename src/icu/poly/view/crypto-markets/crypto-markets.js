...
const DEFAULT_TAG_ID = 21;
let outcomeMode = "updown";
let currentData = [];
...
function getEventEndTimestamp(item) {
    ...
    const endValue = event?.endDate || item?.endDate;
    const timestamp = normalizeSortDate(endValue);
    return Number.isFinite(timestamp) ? timestamp : null;
}

function isExcludedIntervalIdentifier(value) {
    if (typeof value !== "string") return false;
    const normalized = value.trim().toLowerCase();
    if (!normalized) return false;
    return EXCLUDED_INTERVAL_PATTERN.test(normalized);
}

function shouldExcludeMarket(item) {
    const event = Array.isArray(item?.events) ? item.events[0] : null;
    const slugSources = [
        typeof event?.slug === "string" ? event.slug : null,
        typeof item?.slug === "string" ? item.slug : null,
    ];
    if (slugSources.some(isExcludedIntervalIdentifier)) return true;
    const ticker = extractEventTicker(item);
    if (isExcludedIntervalIdentifier(ticker)) return true;
    return false;
}

function filterExcludedMarkets(list) {
...
function buildEventGroupLabel(timestamp, count = 1) {
    const baseLabel = `结束：${formatDateToCST(timestamp)}`;
    if (!count || count <= 1) return baseLabel;
    return `${baseLabel} · ${count} 事件`;
}

function pickNearestEventGroupKey(options) {
    if (!Array.isArray(options) || !options.length) return null;
    const now = Date.now();
    const upcoming = options.find((option) => option.timestamp >= now);
    if (upcoming) return upcoming.key;
    return options[options.length - 1].key;
}

function buildEventGroupOptions(list) {
    const eventMap = new Map();
    list.forEach((item) => {
        if (shouldExcludeMarket(item)) return;
        const ticker = extractEventTicker(item);
        if (!ticker || eventMap.has(ticker)) return;
        if (isExcludedIntervalIdentifier(ticker)) return;
        const timestamp = getEventEndTimestamp(item);
        if (timestamp === null) return;
        eventMap.set(ticker, { ticker, timestamp });
    });
    const groupMap = new Map();
    eventMap.forEach(({ ticker, timestamp }) => {
        ...
        groupMap.get(key).tickers.add(ticker);
    });
    return [...groupMap.values()]
        .map((group) => ({
            key: group.key,
            timestamp: group.timestamp,
            count: group.tickers.size,
            label: buildEventGroupLabel(group.timestamp, group.tickers.size),
        }))
...
function updateEventFilterOptions(list) {
    ...
    const options = buildEventGroupOptions(list);
    eventSelect.innerHTML = "";
    if (!options.length) {
        const opt = document.createElement("option");
        opt.value = "";
        opt.textContent = "暂无结束组";
        eventSelect.appendChild(opt);
        eventSelect.disabled = true;
        currentEventGroupKey = null;
        return;
    }
    ...
    if (!currentEventGroupKey || !options.some((opt) => opt.key === currentEventGroupKey)) {
        currentEventGroupKey = pickNearestEventGroupKey(options);
    }
    eventSelect.value = currentEventGroupKey;
...
function filterByEventGroup(list) {
    if (!currentEventGroupKey) return list;
    return list.filter((item) => {
        const timestamp = getEventEndTimestamp(item);
        if (timestamp === null) return false;
        return String(timestamp) === currentEventGroupKey;
    });
}
...
function collectYesTokens(list) {
    ...
    return tokens.map((token) => ({
        ...
        label: buildFallbackLabel(
            token,
            token.tokenId,
            normalizeTickerLabel(token.ticker ?? token.question),
        ),
...
function calculateSpread(bestAsk, bestBid) {
    const ask = typeof bestAsk === "number" ? bestAsk : Number(bestAsk);
    const bid = typeof bestBid === "number" ? bestBid : Number(bestBid);
...
function calculateTopPrice(quotes) {
...
function ensureOrderBooksForTokens(tokenIds) {
    ...
    tokenIds.forEach((tokenId) => {
        if (!tokenId) return;
        if (
            hasTokenBestQuotes(tokenId) ||
            tokenOrderBookRequests.has(tokenId) ||
            tokenOrderBookQueued.has(tokenId)
        )
            return;
...
function setChartWindowInfo(start, end) {
    ...
    const isFullRange =
        chartViewSpan >= 0.999 || (chartTimeBounds && chartTimeBounds.max <= chartTimeBounds.min);
...
function drawChart(series) {
    ...
    const times = Array.from(new Set(series.flatMap((set) => set.points.map((p) => p.time)))).sort(
        (a, b) => a - b,
    );
    ...
    const yForPrimary = (price) =>
        padding.top + chartHeight * (1 - (clampPrice(price) - priceMin) / priceRange);
...
function drawTimelineEmpty(ctx, metrics) {
    ...
    ctx.fillText(
        "等待数据加载…",
        metrics.padding + metrics.usableWidth / 2,
...
function drawTimelineTicks(ctx, metrics, minTime, maxTime) {
    ...
    const { primaryInterval, secondaryInterval } = chooseTimelineIntervals(
        minTime,
        maxTime,
        metrics.usableWidth,
    );
...
function chooseTimelineIntervals(minTime, maxTime, width) {
    ...
    const secondary =
        intervalMs >= 2 * SNAP_INTERVALS[0] ? Math.max(SNAP_INTERVALS[0], intervalMs / 2) : 0;
...
function getWindowRatios() {
    ...
    const bounds = getViewWindowBounds() || {
        start: chartTimeBounds.min,
        end: chartTimeBounds.max,
    };
    const range = Math.max(1, chartTimeBounds.max - chartTimeBounds.min);
    ...
    for (let i = 0; i <= steps; i++) {
        ...
        if (secondaryScale) {
            const secondaryValue =
                secondaryScale.max - ((secondaryScale.max - secondaryScale.min) / steps) * i;
            ctx.textAlign = "left";
...
function applyChartHover(target, pointer) {
    ...
    if (chartHoverLabelPrice)
        chartHoverLabelPrice.textContent = `价格：${formatChartHoverPrice(dataset, point.price)}`;
...
function normalizeOrderbookPrice(value) {
    if (typeof value === "number") return Number.isFinite(value) ? value : null;
    const numeric = Number(value);
    return Number.isFinite(numeric) ? numeric : null;
}

function getBestAskFromItem(item, outcomeIndex) {
    if (!item) return null;
    const key = outcomeIndex === 0 ? "yesBestAsk" : "noBestAsk";
    return normalizeOrderbookPrice(item?.[key]);
}

function getBestBidFromItem(item, outcomeIndex) {
    if (!item) return null;
    const key = outcomeIndex === 0 ? "yesBestBid" : "noBestBid";
    return normalizeOrderbookPrice(item?.[key]);
}
...
function getCurrentEventGroupLabel() {
    if (!currentEventGroupKey) return null;
    const timestamp = Number(currentEventGroupKey);
    if (!Number.isFinite(timestamp)) return null;
    return formatDateToCST(timestamp);
}

function updateStatus(visibleCount) {
    if (!statusEl) return;
    const loadedCount = filterExcludedMarkets(currentData).length;
    if (!loadedCount) {
    ...
    const label = getTagLabel(currentTagId);
    const parts = [`已载入 ${loadedCount} 条`, `显示 ${visibleCount} 条`];
    if (label) parts.push(`标签：${label}`);
    const eventGroupLabel = getCurrentEventGroupLabel();
    if (eventGroupLabel) parts.push(`结束时间：${eventGroupLabel}`);
    statusEl.textContent = parts.join("，");
}

function refreshView({ skipChart = false } = {}) {
    const sanitized = filterExcludedMarkets(currentData);
    ...
    ensureOrderBooksForMarkets(ordered);
    if (!skipChart) {
        refreshYesTokenChart(ordered);
    }
}
...
if (eventSelect) {
    eventSelect.addEventListener("change", (evt) => {
        currentEventGroupKey = evt.target.value || null;
        refreshView();
    });
}
...
if (orderForm) {
    orderForm.addEventListener("submit", async (evt) => {
        ...
        const takeProfitPrice = orderTakeProfitInput.value
            ? parsePriceInput(orderTakeProfitInput.value)
            : null;
