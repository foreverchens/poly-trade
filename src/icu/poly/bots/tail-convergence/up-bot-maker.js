...
import { resolvePositionSize } from "./common.js";
...
export async function submitMakerSignal(signal) {
    ...
    const size = client.getUsdcEBalance();
    if(size <= 10) {
        logger.error(`[submitMakerSignal] ${symbol}余额为${size}USDC、不足10USDC、无法提交maker单`);
        return;
    }
    logger.info(`[submitMakerSignal] ${symbol}提交maker单、price=${price}、size=${size}、outcome=${outcome}、tokenId=${tokenId.slice(0, 6)}...、`);
    const order = await client.placeOrder(price, size,PolySide.BUY, tokenId).catch((err) => {
        logger.error(`[submitMakerSignal] ${symbol}提交maker单失败、price=${price}、size=${size}、tokenId=${tokenId.slice(0, 6)}...、outcome=${outcome}、error=${err?.message ?? err}`);
        return null;
    });
    if(!order?.success) {
        logger.error(`[submitMakerSignal] ${symbol}提交maker单失败、price=${price}、size=${size}、tokenId=${tokenId.slice(0, 6)}...、outcome=${outcome}、error=${order?.error?.message ?? order?.error}`);
        return;
    }
    signal.orderId = order.orderID;
    logger.info(`[submitMakerSignal] ${symbol}提交maker单成功、price=${price}、size=${size}、tokenId=${tokenId.slice(0, 6)}...、outcome=${outcome}、orderId=${signal.orderId}`);
    // 异步处理挂单、等待成交
...
async function handleMakerOrder(signal) {
    ...
    do {
        // 如果订单不存在、直接结束
        if(!order) {
            logger.info(`[handleMakerOrder] ${symbol}挂单不存在、orderId=${orderId}`);
            return;
        }
        ...
        if(matchedSize === originalSize) {
            logger.info(`[handleMakerOrder] ${symbol}挂单已完全成交、orderId=${orderId}`);
            return;
        }
        if(matchedSize > 0) {
            logger.info(`[handleMakerOrder] ${symbol}挂单部分成交、orderId=${orderId}`);
        }
        // 挂单完全未成交
        // 查询最优bid、如果最优bid高于marker price超过0.002、取消挂单
        const [yesBid] = await client.getBestPrice(tokenId);
        if(yesBid > price + 0.002) {
            logger.info(`[handleMakerOrder] ${symbol}最优bid高于marker price超过0.002、取消挂单、orderId=${orderId}`);
        ...
        if(currentHour !== currentLoopHour && currentMinute >= 20) {
            ...
            logger.info(`[handleMakerOrder] ${symbol}当前小时不等于挂单小时、且当前分钟数大于等于20、取消挂单、orderId=${orderId}`);
            await client.cancelOrder(orderId);
            return;
        }
    } while (false);
    logger.info(`[handleMakerOrder] ${symbol}挂单未完全成交、继续查询、orderId=${orderId}`);
    setTimeout(() => {
        handleMakerOrder(signal);
    }, 30000);
}
